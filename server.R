...
text_raw_data <- read.csv('/Users/fanouyang/Desktop/sis-la/all_code.csv', header=TRUE)
...
function(input, output) {
  ...
  # student to student
  output$stos <- visNetwork::renderVisNetwork({
    if (input$week==1){
      
      persons_data_1 <- text_raw_data %>% filter(week == 'Week 02') %>% select('vert1_id','vert2_id') 
      graph_persons_1 <- igraph::graph_from_data_frame(persons_data_1)
      graph_persons_vis_1 <- visNetwork::toVisNetworkData(graph_persons_1)
      ...
      graph_persons_2 <- igraph::graph_from_data_frame(persons_data_2)
      graph_persons_vis_2 <- visNetwork::toVisNetworkData(graph_persons_2)
      ...
      graph_persons_3 <- igraph::graph_from_data_frame(persons_data_3)
      graph_persons_vis_3 <- visNetwork::toVisNetworkData(graph_persons_3)
  ...
  # student to term
  output$stot <- visNetwork::renderVisNetwork({
    if (input$week==1){
      ...
      text_person_words_1 <- text_person_words_1 %>%
        tidytext::unnest_tokens(bigram,messages,token = 'ngrams',n=2)
      ...
      graph_person_words_1 <- igraph::graph_from_data_frame(text_person_words_unite_1)
      ...
      graph_person_words_2 <- igraph::graph_from_data_frame(text_person_words_unite_2)
      
      graph_person_words_vis_2 <- visNetwork::toVisNetworkData(graph_person_words_2)
      #color
      ...
      graph_person_words_3 <- igraph::graph_from_data_frame(text_person_words_unite_3)
      
      graph_person_words_vis_3 <- visNetwork::toVisNetworkData(graph_person_words_3)
      #color
  ...
  # term to term
  output$ttot <- visNetwork::renderVisNetwork({
    if (input$week==1){
      text_raw_data_1 <- text_raw_data %>% filter(week == 'Week 02')
      text_message_person_1 <- as_tibble(text_raw_data_1[1:3])
      ...
      text_bigram_count3_1 <- text_bigram_count2_1 %>%
        filter(n>=3)
      text_bigram_graph_1 <- igraph::graph_from_data_frame(text_bigram_count3_1) 
      
      # this graph is so not beautiful,we choos another tool for visualization
      library(visNetwork)
      text_vis_data_1 <- visNetwork::toVisNetworkData(text_bigram_graph_1)
      text_bigram_graph_color_1 <- generateVcolor2(as.integer(igraph::degree(text_bigram_graph_1)))
      ...
      degree_value_1 <- igraph::degree(text_bigram_graph_1,mode='all')
      ...
      text_bigram_count3_2 <- text_bigram_count2_2 %>%
        filter(n>=3)
      text_bigram_graph_2 <- igraph::graph_from_data_frame(text_bigram_count3_2) 
      
      # this graph is so not beautiful,we choos another tool for visualization
      library(visNetwork)
      text_vis_data_2 <- visNetwork::toVisNetworkData(text_bigram_graph_2)
      ...
      text_bigram_count3_3 <- text_bigram_count2_3 %>%
        filter(n>=3)
      text_bigram_graph_3 <- igraph::graph_from_data_frame(text_bigram_count3_3) 
      
      # this graph is so not beautiful,we choos another tool for visualization
      library(visNetwork)
      text_vis_data_3 <- visNetwork::toVisNetworkData(text_bigram_graph_3)
  ...
  output$info <- renderUI({
    ...
    a("Shiny app designed by Fan Ouyang and Xu Li", href="https://github.com/fanouyang/sna_exampler", target="_blank")
